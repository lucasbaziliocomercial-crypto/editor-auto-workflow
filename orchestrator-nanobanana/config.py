...
DEFAULTS = {
    ...
    # --- BACKEND DE IMAGEM DO CORPO (Etapa 5) — CÓPIA WAVESPEED (Seedream) ------
    # Esta cópia (orchestrator-nanobanana) gera as imagens do CORPO no WAVESPEED (modelo Seedream,
    # via MCP OFICIAL) em vez do Magnific. O s5_imagens desta pasta importa `wavespeed_seam` no
    # lugar do `magnific_seam`; estas envs configuram-no. A esteira ORIGINAL (orchestrator/) ignora
    # tudo isto e segue no Magnific. Ver TESTAR-WAVESPEED.ps1.
    "ROTEIRO_IMG_BACKEND": "wavespeed",      # marcador (informativo) — qual backend esta cópia usa
    "WAVESPEED_MCP": "mcp__wavespeed",       # prefixo do servidor MCP OFICIAL do WaveSpeed
    "WAVESPEED_MODEL": "seedream-v5.0-lite", # informativo (o endpoint real vem das envs do MCP)
    # WAVESPEED_API_KEY NÃO tem default aqui de propósito: você exporta (do painel WaveSpeed). O
    # WaveSpeed COBRA por imagem (~$0,035 Seedream 5 Lite) — confirme saldo/plano na conta. O
    # endpoint do modelo Seedream é definido no registro do MCP (WAVESPEED_API_*_ENDPOINT).
